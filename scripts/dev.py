...
def lint_code():
    ...
    if not run_command("uv run ruff format --check src tests", "Ruff format check"):
        success = False
...
def run_security_tests():
    ...
    return run_command("uv run python -m unittest tests.test_security -v", "Security tests")
...
def check_dependencies():
    ...
    if not run_command("uv run python -c 'import kartezio; print(\"✅ Package imports successfully\")'", 
                      "Package import test"):
        success = False
...
def main():
    ...
    parser.add_argument("command", choices=[
        "lint", "format", "test", "security", "typecheck", 
        "deps", "build", "clean", "install", "all"
    ], help="Command to run")
    parser.add_argument("--fast", action="store_true", help="Skip slow operations")
