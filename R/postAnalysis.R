...
for (maxi in maxis) {
  ...
  # rnd = load_al(filename = "Documents/MCF/results/random/_oneplus_nMut_1_nDiv_10/raw_test_data.txt",
  #                   name="random",
  #                   maxi = maxi)
  ...
  train_data = rbind(cluster,ppsnlike,typical)
...
for (algo in unique(tmp$algorithm)){
  cat(mean(tmp[tmp$algorithm==algo,]$test),sd(tmp[tmp$algorithm==algo,]$test),'\n')
}
